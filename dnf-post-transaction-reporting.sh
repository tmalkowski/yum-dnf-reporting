...
if [[ $# -ne 7 ]]; then
	echo "This script is meant to be run via dnf-plugins." >&2
	exit 1
fi
timestamp=$( date '+%s.%3N' )
STATE=$1
...
json=$( printf '{"time":"%s","host":"%s","source":"dnf-plugins","event":{"state":"%s","name":"%s","epoch":"%s","version":"%s","release":"%s","arch":"%s","repoid":"%s"}}' "$timestamp" "$HOSTNAME"   "$STATE" "$NAME" "$EPOCH" "$VERSION" "$RELEASE" "$ARCH" "$REPOID" )
